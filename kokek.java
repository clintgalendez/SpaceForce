...
public class kokek {
    ...
    static List<Integer> cachePos = new ArrayList<>();
    
    static Scanner userScanner = new Scanner(System.in);
    static String userInput = "";
    static String playerName = "";

    static int indexOfChosenInput = 0;
    static int userCol;
    static int userRow;
    static int cpuCol;
    static int cpuRow;
    static int chosenPlace;
    ...
    public static void inputCheck() {
        ...
        indexOfChosenInput = Integer.parseInt(Integer.toString(userRow) + Integer.toString(userCol));
        

        while (indexOfChosenInput >= boardPlayer.size() || boardPlayer.get(indexOfChosenInput).equals("@") || boardPlayer.get(indexOfChosenInput).equals("X") || boardPlayer.get(indexOfChosenInput).equals("-")) {
    ...
    public static void cpuMove() {
        ...
        chosenPlace = Integer.parseInt(Integer.toString(cpuRow) + Integer.toString(cpuCol));
        while (cpuPos.contains(chosenPlace) || boardEnemy.get(chosenPlace).equals("X") || boardEnemy.get(chosenPlace).equals("-")) {
            cpuMove();
        }     
    }
    ...
    public static void gamePlan() {
        ...
        for (int i = 1; i < 21; i++) {
            ...
            inputCheck();
            boardPlayer.set(indexOfChosenInput, "@");
            playerPos.add(indexOfChosenInput);
        }

        for (int i = 1; i < 21; i++) {
            cpuMove();
            cpuPos.add(chosenPlace);
        }
    }
    
    public static void gameScore(boolean defeatEnemy, boolean changeScore) {
    ...
    public static void mainGame() throws FileNotFoundException {
        ...
        while (checkWinner().equals("")) {
            if (isPlayer) {
                ...
                showMessageBox("");
                moveCheck(boardPlayer, cpuPos, indexOfChosenInput);
                ...
                moveCheck(boardEnemy, playerPos, chosenPlace);
        ...
        if (checkWinner().equals("player")) {
            clearConsole();
            System.out.println("Congratulations! You've won! with total points of: " + (currentScore - move));
            ...
            System.out.println("Sorry, You lost! " + (currentScore - move));
        } else {
